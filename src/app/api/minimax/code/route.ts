...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 统一使用 Anthropic 兼容的 MiniMax M3 文本模型。
    const { messages } = body;
    ...
    const systemPrompt = {
      ...
      name: 'system',
      content: `根据用户需求生成一个可直接运行的完整单文件 HTML（内含 CSS 和 JavaScript）。必须有真实内容和可操作交互，不能是空壳；表单需有校验和提交反馈。只输出从 <!doctype html> 开始的 HTML 源码，不要 Markdown、讲解或优化建议。不要引用外部随机图片。`
    };
    ...
    const payload = toM3Payload(finalMessages, { maxTokens: 4096, temperature: 0.3 });
    const headers = minimaxM3Headers();
    ...
    const html = extractMinimaxHtml(content);
    if (!html) return NextResponse.json({ error: "AI 返回内容不是可预览的 HTML。", code: "PROVIDER_INVALID_RESPONSE", retryable: true }, { status: 502 });
