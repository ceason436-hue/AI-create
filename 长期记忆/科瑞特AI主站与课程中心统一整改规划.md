...
- `[IN_PROGRESS / Phase 1]` 已建立课程分类、课程、模块、课时、报名、课程工具绑定、课件、内容、媒体和审计所需 Prisma 模型、首个增量迁移、公开 API、管理员 API 与后台入口。
...
- `[IN_PROGRESS / Phase 6]` 已建立 `/tools` 双入口目录、旧工具 URL 重定向和匿名试用同网关受控分支；匿名请求使用随机请求标识并写入脱敏用量审计，不进入个人账户、作品或学习记录；Redis、供应商凭据和真实账户权益仍需部署环境验收，试用日历按 `AI_TRIAL_TIMEZONE` 配置（默认 `Asia/Shanghai`）。
...
- `[IN_PROGRESS / Phase 2]` 已实现本地开发回退与私有 Aliyun OSS REST 存储驱动、课件原件受控上传、原件/预览对象分区、受保护预览会话、动态水印、处理队列、LibreOffice 转 PDF 处理端点及失败重试；生产环境仍需配置 OSS RAM、`COURSEWARE_WORKER_TOKEN`、定时调度和 LibreOffice，并以真实课件完成版式验收。
- `[IN_PROGRESS / Phase 3]` 运营后台现支持课件上传、预览处理状态、送审、发布、下线和失败重试；课程、报名和内容 CMS 的基础 CRUD 与审计已具备，完整内容版本恢复和真实运营数据录入仍需继续完成。
- `[PENDING / Phase 7]` 旧内容删除清单和正式上线收口仍需继续完成；在这些事项完成前不得宣称全部整改完成。
...
- `[LIVE]` 个人作品中心与本地存储适配层。
- `[PLANNED]` OSS 配置项已经预留，但 OSS 存储驱动尚未实现。
...
6. 当前存储驱动仅实现本地文件，课程课件上线前 OSS 驱动、RAM 权限和转换 Worker 必须完成。
7. 当前数据库没有课程和报名实体，不能只用培训班或套餐临时替代。
