...
export function ReferenceArt() {
  ...
  const [mode, setMode] = useState<"text2img" | "img2img">("text2img"),
    ...
    [composition, setComposition] = useState<string | null>(null),
    [customComposition, setCustomComposition] = useState(""),
    ...
    [results, setResults] = useState<Work[]>([]),
  ...
  async function currentReference() {
    if (!current) throw new Error("请先生成或打开一张作品。");
  ...
  async function generate(variant = false) {
    ...
    // The provider returns a final response instead of server-sent progress.
    // Keep this estimate below completion until a real result is received.
    const progressTimer = window.setInterval(() => {
      setGenerationProgress((value) =>
        value >= 92 ? value : Math.min(92, value + (value < 48 ? 5 : 2)),
      );
    }, 700);
    try {
      ...
      for (let i = 0; i < count; i++) {
          ...
          body: JSON.stringify({
            ...
            prompt: [
              ...
              composition || customComposition.trim()
                ? `构图：${customComposition.trim() || composition}`
                : "",
              avoid ? `避免：${avoid}` : "",
            ]
              .filter(Boolean)
        ...
        generated.push(work);
        setGenerationProgress(Math.max(12, Math.round(((i + 1) / count) * 96)));
        if (personal) {
          records.current = [];
          setWorks([]);
  ...
  function save() {
    if (!current) return setNotice("请先生成或打开一张作品。");
  ...
  return (
    <main className={s.page} aria-label="AI绘画创作工作台">
      <button className={s.back} onClick={() => history.back()}><ChevronLeft />返回</button>
      <div className={s.workspace}>
        <section className={s.form} aria-label="绘画创作设置">
          ...
          <fieldset className={s.field}>
            <legend>风格参考 <small>（可选）</small></legend>
            <details className={s.picker}>
              <summary>
                <span>{style || "不使用风格参考"}</span>
                <small>{style ? "已选择，可随时取消" : `展开查看 ${styleOptions.length} 种参考`}</small>
                <ChevronDown />
              </summary>
              <div className={s.styleList}>
                {styleOptions.map((option) => (
                  <button
                    ...
                    onClick={() =>
                      setStyle(style === option.name ? null : option.name)
                    }
                  >
                    <ReferenceAsset
                      src={option.src}
          ...
          <fieldset className={s.field}>
            <legend>构图指南 <small>（可选）</small></legend>
            <div className={s.compositions}>
              {compositions.map((c, i) => (
                <button
                  ...
                  onClick={() => {
                    setComposition(composition === c.name ? null : c.name);
            ...
            <div className={s.customComposition}>
              <label htmlFor="custom-composition">或自定义构图要求</label>
              <textarea
                id="custom-composition"
                value={customComposition}
                maxLength={200}
                placeholder="例如：主体靠右，左侧留出文字空间（可留空）"
                onChange={(event) => {
                  setCustomComposition(event.target.value);
                  if (event.target.value) setComposition(null);
                }}
              />
            </div>
          </fieldset>
          ...
          {tab === "当前生成" ? (
            <>
              <div className={s.canvas}>
                {current ? (
                  <button className={s.artPreview} onClick={() => setPreviewOpen(true)} aria-label="放大查看生成的图片"><Artwork work={current} /></button>
                ) : (
                  <div className={s.emptyCanvas}>
                    <ImageIcon />
                    <h2>还没有生成画面</h2>
                    <p>填写创作描述后开始生成，结果将显示在这里。</p>
                  </div>
                )}
                {busy && <div className={s.generating} role="progressbar" aria-label="AI 绘画生成进度" aria-valuemin={0} aria-valuemax={100} aria-valuenow={generationProgress}><div className={s.loadingMascot}><Sparkles /></div><strong>{generationProgress}%</strong><span>{generationProgress < 40 ? "小画笔正在听你的想法…" : generationProgress < 82 ? "正在给画面涂上魔法颜色…" : "马上把画面送到你面前！"}</span></div>}
              </div>
              ...
              <div className={s.actions}>
                <button onClick={compare} disabled={!current}>
                  <Scale />
                  <span>
                    <strong>比较所选</strong>
                    <small>对比差异</small>
                ...
                <button onClick={() => generate(true)} disabled={busy || !current}>
                  <Box />
                  <span>
                    <strong>生成变体</strong>
                    <small>基于所选图生成</small>
                ...
                <button onClick={edit} disabled={busy || !current}>
                  <Pencil />
                  <span>
                    <strong>继续编辑</strong>
                ...
                <button className={s.save} onClick={save} disabled={!current}>
                  <Download />
                  <span>
                    <strong>保存作品</strong>
                    <small>下载到本地</small>
      ...
      {previewOpen && current && <div className={s.lightbox} role="dialog" aria-modal="true" aria-label="放大查看图片" onClick={() => setPreviewOpen(false)}><button aria-label="关闭图片预览">关闭 ×</button><Artwork work={current} /></div>}
    </main>
  );
}
