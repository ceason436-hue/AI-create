...
export async function getPublicCourses(filters?: { category?: string; query?: string }) {
  if (isTemporaryDeployment()) return [];
  try {
...
export async function getPublicCourse(slug: string) {
  if (isTemporaryDeployment()) return null;
  try {
...
async function getPublishedList(model: "activity" | "achievement" | "teacherProfile" | "campus") {
  if (isTemporaryDeployment()) return model === "activity" ? fallbackActivities : model === "achievement" ? fallbackAchievements : model === "teacherProfile" ? fallbackTeachers : fallbackCampuses;
  try {
...
export async function loadPublicCourseCatalog() {
  if (isTemporaryDeployment()) return { state: "unavailable" as const, categories: undefined, courses: undefined };
  try {
