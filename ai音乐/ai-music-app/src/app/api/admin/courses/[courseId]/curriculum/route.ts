...
const toolKey = z.string().refine(isKnownAiTool, "工具不在服务端白名单中。");
...
export async function PATCH(request: Request, { params }: { params: Promise<{ courseId: string }> }) {
  ...
  try {
    ...
    let item: unknown;
    if (kind === "MODULE") { const courseModule = await db.courseModule.findFirst({ where: { id, courseId }, select: { id: true } }); if (!courseModule) return Response.json({ error: "模块不属于当前课程。" }, { status: 404 }); item = await db.courseModule.update({ where: { id }, data }); }
    ...
    if (kind === "TOOL_BINDING") { const binding = await db.courseToolBinding.findFirst({ where: { id, courseId }, select: { id: true } }); if (!binding) return Response.json({ error: "工具绑定不属于当前课程。" }, { status: 404 }); if ("toolKey" in data && data.toolKey) { const tool = await db.aiTool.findFirst({ where: { toolKey: data.toolKey, status: "ACTIVE" }, select: { id: true } }); if (!tool) return Response.json({ error: "该 AI 工具已暂停，不能更新为课程绑定。" }, { status: 409 }); } item = await db.courseToolBinding.update({ where: { id }, data }); }
