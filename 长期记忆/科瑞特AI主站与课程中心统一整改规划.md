...
> 正式应用：`D:\programe\AI科瑞特\AI-create`
...
- `[DONE / Phase 5 foundation]` 已建立 `/learn` 学习中心、报名有效期校验、学习进度接口、课程课时页和受限课件预览入口；学习首页现汇总继续课时、即将到期报名、最近查看课件、课程内 AI 工具和最近个人作品。作品当前仍是账户维度，尚未伪造课程归属。
...
- `[IN_PROGRESS / Phase 3]` 运营后台现支持课件上传、预览处理状态、送审、发布、下线和失败重试；新增 `/admin/course-categories` 供运营人员维护课程一级/后续扩展分类的名称、介绍、排序和启用状态，`/admin/curriculum` 维护课程模块、课时、课时任务和 AI 工具绑定，并新增 `/admin/inquiries` 统一筛选和处理课程咨询、校园合作线索，状态变更写入审计。新增 `/admin/site-pages` 页面区块 CMS：可维护页面、区块、排序、主题、发布状态和 JSON 内容载荷；每次保存或恢复都会生成内容版本，可从历史版本恢复，首页与校园合作页已读取已发布区块并在数据库不可用时保留静态回退。新增 `/admin/media` 图片媒体库，管理员可上传受限格式图片，登记真实/手册/生成/占位来源、替代文本与授权说明，并将媒体资源绑定为活动、成果、师资头像、校区封面或合作学校 Logo；公开页通过已启用资源的受控 `/api/media/:id` 地址读取，活动、成果、师资、校区与校园合作页面改为动态读取已发布内容。视频兼容转码、海报与人工 WebVTT 字幕的代码基础已落地，HLS、多码率派生、自动转写、生产调度和真实运营数据授权核验仍待完成。
- `[DONE / Phase 3 media publish control]` 图片媒体库新增媒体下线与恢复操作；状态变更会写入审计，已下线资源通过公开媒体路由会立即返回不可用，从而不再在前台显示。
- `[DONE / Phase 3 public video]` 媒体库现支持 MP4 与 WebM 的受控上传、文件签名校验、公开媒体代理和活动/成果详情播放；图片限制为 15MB，视频默认限制为 200MB（可由 `MEDIA_VIDEO_UPLOAD_MAX_BYTES` 收紧）。已实现异步兼容 MP4 转码、封面抽帧，以及私有 WebVTT 字幕上传/受控播放的 Worker 与后台代码；HLS、自动转写和多尺寸派生仍待媒体服务选型与生产接入。
...
- `[DONE / Dynamic public sitemap]` `/sitemap.xml` 现除固定公开页外，还只收录运营后台已发布的课程、活动和成果详情，并使用各内容的更新时间；数据库暂不可用时安全退回固定公开页，不会把前端占位内容、草稿或已移除示例课程写入站点地图。真实生产数据恢复后仍需复验动态详情 URL。
...
- `[DONE / Learning course context]` 课程学习页现展示有效期、报名来源、学习进度、课程内已启用工具与个人作品入口；工具只有绑定到仍可访问课时才提供课时入口，课程级工具提示从课时任务打开，避免在前端伪造课程/课时上下文或绕过服务端校验。课程公告和课程作品归属尚无独立数据模型，不以占位内容冒充已实现能力。
- `[DONE / Learning progress writes]` 学员课时页现提供“标记本课已完成/撤销完成”操作；服务端仅在个人学员、有效报名和已发布课时条件同时成立时更新 `LearningProgress`，并在同一事务内按已发布课时重新计算课程进度、记录最后学习课时与审计日志。真实账户下的完成/撤销、并发写入和进度显示仍待数据库环境恢复后验收。
- `[DONE / Handbook source labeling]` 公开页现区分手册真实资料、已授权真实媒体、生成概念图与品牌占位，避免把已导出的手册第 5、7、8 页错误标为“品牌占位”。“走进科瑞特”已使用第 5 页的经核验品牌资料；校园合作页记录第 65 页的校企合作 Logo/课堂资料来源，并继续只展示后台中已标注授权且发布的单个合作学校 Logo，避免批量素材未经逐项授权直接公开。
- `[DONE / Handbook web-asset audit]` 已核验正式应用当前引用的 `/handbook/handbook-05.png`、`handbook-07.png`、`handbook-08.png` 均存在于受管理的 `public/handbook/`，不会出现手册资料页面缺图；已导出的第 10–12 页素材暂未被页面引用，继续保留为后续运营素材，原始 PDF 与导出图均不删除。
- `[DONE / Placeholder provenance labeling]` 活动、成果、师资和校区的品牌占位回退现均显式携带“品牌占位内容 · `[PENDING-CONTENT]`”来源标签；活动/成果卡片类型也标明“占位”。手册第 5、7、8 页资料继续保留真实资料与待确认标签，避免将概念内容、待补资料或手册资料混淆为已授权真实活动、作品、教师、校区或获奖事实。已在真实开发应用验证桌面和 390px 移动端呈现。
- `[DONE / Evidence-based acceptance matrix]` 已新增《科瑞特AI统一整改验收矩阵》，按公开主站、课程学习、AI 工具、课件媒体、旧内容和质量门槛逐项区分本地验证、环境待验收、负责人待确认和删除待确认；后续线程不得以本规划或口头状态替代其中列明的实际验收条件。
- `[DONE / Course announcements, work context and governed tool catalog]` 项目负责人已确认《课程公告作品与AI工具模型确认提案》的三项最小范围。已新增 `CourseAnnouncement`、`WorkCourseContext` 和受服务端白名单约束的 `AiTool`，并完成两项受控迁移：`20260902100908_course_announcements_work_context_ai_tools`、`20260902101500_seed_ai_tool_catalog`。运营端可维护公告及既有工具的展示/全局启停/匿名试用策略；学员课程页只读有效报名课程的已发布公告和个人作品归属；网关在供应商调用前拒绝未启用工具。课程内由 AI 供应商生成并自动落库为作品的写入链尚未存在，不得把“可安全关联既有个人作品”误报为“所有生成结果已经自动归档”。
- `[DONE / Local Docker and migration verification]` Docker Desktop 服务、PostgreSQL 与 Redis 已恢复；`krt-postgres`、`krt-redis` 健康，9 项 Prisma 迁移（含课程公告、作品归属和工具目录）已部署，本地迁移状态为 up to date。真实管理员/学员/学校账号、私有 OSS、供应商凭据和生产 URL 的端到端验收仍须在相应环境逐项完成。
- `[DONE / Next security update]` 经项目负责人授权，在保留现有未提交依赖改动的基础上将 Next.js 与 ESLint 配套配置升级至 `16.3.4`。生产依赖审计由 8 个高危项降为 4 个，剩余均为 Prisma 配置传递依赖链；未擅自采用审计建议中与当前 Prisma 主版本不一致的降级/重大变更。类型检查、39 项单元测试、凭据扫描与 Webpack 生产构建已通过；当前本机 SWC 原生二进制损坏，构建使用 Next WASM 回退并记录为本机环境修复事项，不代表生产镜像可跳过干净安装验证。
- `[DONE / Lint remediation pass 1]` 已修复后台入口、AI 阅读与 AI 绘画页的首批全量 ESLint 阻断项：主站返回使用 Next `Link`，确认弹窗以组件重挂载而非 effect 内同步 setState 清空密码，阅读历史改为安全的 `useSyncExternalStore` 本地存储订阅，绘画页收紧本地作品及供应商响应类型。全量结果由 `58 errors / 46 warnings` 降至 `53 errors / 45 warnings`；本阶段三个页面均为 0 errors，TypeScript 和 39 项单元测试通过。其余历史 AI 页面与运营组件的 lint 事项将分批修复，不以降低规则或关闭检查掩盖问题。
- `[DONE / Lint remediation pass 2]` AI 阅读结果页已将会话、分段、视觉点评与本地历史解析改为显式类型和运行时校验；不再信任任意 JSON 直接驱动批量下载、点评和导出展示。全量 ESLint 进一步降至 `45 errors / 45 warnings`，该页为 0 errors；TypeScript 与 39 项单元测试继续通过。下一批为 AI 阅读工作区和音乐创建页的历史请求/本地存储代码。
- `[DONE / Lint remediation pass 3]` AI 阅读工作区现对本地会话、段落、聊天记录、配额异常、AI 段落拆分与对话 JSON 进行结构化解析；无效数据会安全拒绝或回退，不再用 `any` 直接进入课程阅读和生图流程。该页 12 个 ESLint 错误已清零；全量结果降至 `33 errors / 43 warnings`，TypeScript 与 39 项单元测试继续通过。后续优先清理音乐创建页与运营组件的剩余错误。
- `[DONE / Lint remediation pass 4]` 音乐创建页的作品、收藏、播放器、歌词时间线和生成失败分支已改为显式 `MusicTrack`/歌词行类型；不再以 `any` 驱动音频链接、播放、下载或作品操作。该页 15 个错误已清零，全量 ESLint 降至 `18 errors / 43 warnings`；TypeScript 与 39 项单元测试继续通过。剩余阻断项集中在历史运营组件，后续继续逐批治理。
- `[DONE / Lint remediation pass 5]` 旋律练习页已为 WebKit AudioContext 兼容入口和旋律音符序列补充明确类型，消除两处 `any`。全量 ESLint 降至 `16 errors / 43 warnings`；TypeScript 与 39 项单元测试继续通过。剩余错误主要为节奏/三键练习及运营媒体组件的历史生命周期实现。
- `[DONE / Lint remediation pass 6]` 课程结构运营 API 中用于归属校验的局部变量已从 CommonJS 保留名 `module` 改为语义明确的 `courseModule`，不改变课程模块、课时和 AI 工具绑定的权限或数据校验。全量 ESLint 降至 `14 errors / 43 warnings`；该 API 的定向 ESLint 检查、TypeScript 与 39 项单元测试均通过。其余错误仍集中在节奏/三键练习及运营媒体组件，不能误报为全量 lint 已通过。
- `[DONE / Lint remediation pass 7]` 三键作曲练习页的 Tone.js 合成器引用与音序步骤现使用 `PlayablePolySynth`、`SequenceNote` 等显式类型；合成器仍只暴露实际调用的播放与释放能力，音符、和弦和休止符均受类型约束。该页两处 `any` 已清零，全量 ESLint 降至 `12 errors / 43 warnings`；定向 ESLint、TypeScript 与 39 项单元测试均通过。剩余错误集中在节奏练习与运营媒体组件，不能误报为全量 lint 已通过。
- `[DONE / Lint remediation pass 8]` 节奏练习页已为 WebKit AudioContext 兼容入口定义显式窗口类型，并在不支持 Web Audio API 时安全中止和记录原因；AI 节奏工具的乐器判定已改为不可变局部值，JSX 引号文本改为受控表达式。未改变节奏生成或播放规则，该页 7 个错误已清零；全量 ESLint 降至 `5 errors / 43 warnings`，定向 ESLint、TypeScript 与 39 项单元测试均通过。剩余错误仅在运营媒体组件，不能误报为全量 lint 已通过。
- `[DONE / Lint remediation pass 9]` 运营媒体库、内容媒体编排、媒体历史、字幕和媒体位页面的初始读取均改为 effect 内受控异步调度，避免 React 16 将初始请求中的状态变更判定为同步级联渲染；上传、恢复、媒体绑定、字幕和媒体位保存后的刷新路径不变。五个运营媒体组件的阻断项已清零，完整 ESLint 现为 `0 errors / 43 warnings`，TypeScript 与 39 项单元测试通过；真实开发应用 `/admin/media` 已验证桌面端表单、素材来源选项和空库回退渲染。当前警告为历史图片优化、依赖数组和未使用符号，后续应逐项治理，但质量门禁已不再被 lint 错误阻断。
- `[DONE / Clean dependency build revalidation]` 已使用锁文件完成本机 `npm ci`，重新执行 `prisma generate` 后以正常 `npm run build`（Next 16.3.4 Turbopack）成功完成 95 个路由的生产构建；此前损坏的本机 SWC 二进制问题已随干净依赖重装消除，不再使用 WASM 回退。`npx tsc --noEmit`、39 项 Vitest 与凭据扫描继续通过。该证据证明锁文件可在干净本机依赖下重建，但 CI/生产镜像、真实凭据和部署环境仍须独立验收。
- `[DONE / Lint hygiene pass 10]` 已移除应用外壳、音高/节奏练习、课件 Worker 路由和学习服务中的无用导入及未使用局部变量，不改变公开导航、节奏生成、课件处理或学习访问逻辑。全量 ESLint 由 `0 errors / 43 warnings` 降至 `0 errors / 33 warnings`；TypeScript 与 39 项单元测试通过。剩余警告主要为图片优化、历史 effect 依赖和字体加载策略，后续需结合运行时行为逐项整改。
...
- `[DONE / Individual enrollment selector and validation]` 单条报名录入现使用仅管理员可访问的账户选择数据接口，接口只返回启用个人账户的最小识别字段（ID、登录标识、显示名称、状态），不会向页面泄露额外账户资料；课程同样从管理员课程接口选择。浏览器本地时间会转换为带时区 ISO 时间后提交，服务端再复用批量导入的账户类型、启用状态、课程存在性和时间范围校验，防止绕过前端伪造报名。已在真实开发应用验证桌面与 390px 移动端无横向溢出，未登录读取账户/提交报名均为 401；真实管理员写入仍待数据库环境验收。
- `[DONE / Course publication safeguards]` 课程后台创建表单已暴露公开介绍、适合人群、年级、难度、授课方式、周期、招生状态和封面媒体等既有字段；课程可先保存草稿，再由课程结构后台维护模块、课时和工具绑定。服务端仅在适合人群、封面和至少一个已发布模块均齐备时允许课程发布；公开课程目录和详情查询也显式排除草稿模块与草稿课时，避免后台未发布结构泄露到公开端。真实管理员下的创建、发布/下线与媒体绑定仍待数据库环境验收。
- `[DONE / Course metadata editing]` 运营后台课程列表已提供“编辑基础信息”路由，可维护已有课程的分类、介绍、适合人群、年级、难度、授课方式、周期、招生状态、封面和发布状态；Slug 保持只读以避免既有公开地址被意外改变。模块、课时与工具绑定继续在独立课程结构页面维护。当前本地缺少管理员会话与课程数据，真实编辑保存仍待数据库环境验收。
- `[DONE / Courseware selector UX]` 课件上传页已从手工填写课程/课时 ID 改为服务端加载的课程与可选课时选择器；选择课程后才可选择其课时，并明确区分课程级课件与课时课件。上传接口仍会在服务端校验课时所属课程，不能通过前端表单伪造归属。真实管理员下的选择、上传、转换和发布验收仍待数据库与私有存储环境恢复。
- `[DONE / Curriculum editing UX]` 课程结构后台现可编辑既有模块、课时与工具绑定：维护标题、说明/任务正文、预计分钟、发布或启用状态与排序；每次保存均复用既有的课程归属验证、字段白名单与审计接口。当前本地缺少真实课程和管理员会话，只验证了页面加载与响应式布局；真实编辑、排序和课程内 AI 工具即时生效仍待数据库环境验收。
- `[DONE / Course category editing UX]` 课程分类后台现可编辑既有分类的名称、介绍、排序和启用/归档状态；分类 Slug 保持只读，避免已发布课程筛选地址被意外改变。已有课程所属分类不提供物理删除路径。当前本地缺少真实分类与管理员会话，只验证了页面加载与响应式布局；真实编辑与公开筛选即时生效仍待数据库环境验收。
- `[DONE / Course content source enforcement]` 已移除公开课程读取在数据库不可用时的具体课程、模块和课时硬编码回退；课程分类、名称、介绍、章节和课件只会由运营后台维护并在已发布后进入公开页。无数据库或无已发布课程时，`/courses` 显示明确的整理中状态与咨询入口，课程详情返回未找到而不展示示例数据。已在真实开发应用验证桌面和 390px 移动端空状态无横向溢出。
- `[DONE / Course entry link cleanup]` 首页“学习过程”区块的默认行动入口已从已移除的示例课程地址改为 `/courses` 通用课程体系目录，默认文案不再引用具体课程名称；数据库中已发布的首页区块仍可由运营后台覆盖文案。已在真实开发应用验证桌面和 390px 移动端入口均指向课程目录且目标页无横向溢出。
- `[DONE / Homepage category source enforcement]` 首页课程方向现在直接读取运营后台已启用的课程分类，而不再由已发布课程反推；因此运营人员可以先维护分类、稍后发布具体课程。无分类时显示明确整理中状态，避免使用前端示例分类；已在真实开发应用验证桌面和 390px 移动端无横向溢出。
- `[DONE / AI reading local sample hygiene]` 已移除 AI 阅读工作区未使用的 mock 分段数据，并将内置阅读示例封面从临时第三方 URL 切换到正式应用本地品牌占位资源；新增回归测试要求内置封面使用受管理的本地路径，不将示例资料误标为真实课堂或外部授权素材。
- `[DONE / Phase 3 video processing foundation]` 视频上传现创建异步处理任务，受保护 Worker 使用 FFmpeg 生成兼容 MP4 与海报对象；公开受控媒体地址仅在处理完成后自动切换至兼容播放源，失败可审计并由运营后台重试。后台还可上传私有 WebVTT 字幕，图集视频播放器通过受控字幕地址加载。迁移 `20260902100000_media_processing`、`20260902101000_media_captions`、`MEDIA_WORKER_TOKEN`、FFmpeg、私有存储、任务调度和真实 MP4/WebM/字幕浏览器验收仍待生产环境完成；HLS 自适应流、自动转写与多码率派生未被误报为已完成，详见《媒体视频处理部署清单》。
...
- 检查根目录是否存在历史应用残留，确认无脚本、部署和编辑器配置依赖后清理。
...
8. 视频处理任务、兼容 MP4、海报生成和人工 WebVTT 字幕已实现；生产仍需 FFmpeg、私有对象存储、`MEDIA_WORKER_TOKEN`、调度器和真实授权 MP4/WebM/字幕验收，HLS、自动转写和多码率不在当前完成声明内。
9. 工作区已有大量未提交修改，实施时必须小范围提交并避免覆盖现有代码。
10. 登录后默认回首页的既有行为需要在学员中心完成后再改为 `/learn`，避免提前产生空页面。
