...
export default function PitchExplorer() {
  ...
  const playNote = async (pitch: typeof PITCHES[0]) => {
    ...
    if (synthRef.current) {
      synthRef.current.triggerAttackRelease(pitch.note, "4n", now);
    }

    // Use Tone.Draw for UI sync
    Tone.Draw.schedule(() => {
  ...
  const playSequence = async (direction: "up" | "down") => {
    ...
    sequence.forEach((pitch, index) => {
      const scheduledTime = now + index * noteInterval;
      
      if (synthRef.current) {
        synthRef.current.triggerAttackRelease(pitch.note, "8n", scheduledTime);
      }
      ...
      // If it's the last note, schedule the cleanup
      if (index === sequence.length - 1) {
        const endTime = scheduledTime + 0.4;
        Tone.Draw.schedule(() => {
          setActiveNote(null);
