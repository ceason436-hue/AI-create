...
function aiTextFromResponse(value: unknown) { const result = isRecord(value) && isRecord(value.result) && isRecord(value.result.preview) ? value.result.preview : null; return result && typeof result.text === "string" ? result.text : ""; }
function aiImageFromResponse(value: unknown) { const result = isRecord(value) && isRecord(value.result) && isRecord(value.result.preview) ? value.result.preview : null; return result && typeof result.image === "string" ? result.image : ""; }
...
function AIReadingWorkspaceContent() {
  ...
  useEffect(() => {
    if (!isLoaded || urlSessionId || !selectedArticle?.segments?.length) return;
    const presetSegments = selectedArticle.segments.map(normalizeSegment).filter((item): item is ReadingSegment => item !== null);
    if (!presetSegments.length) return;
    const presetSessionId = `preset_${selectedArticle.id}`;
    setSessionId(presetSessionId);
    setSegments(presetSegments);
    setCurrentIndex(0);
    setChatHistory([{ role: "ai", content: presetSegments[0].question }]);
    setIsManualInput(false);
    window.history.replaceState(null, "", `?sessionId=${presetSessionId}`);
  }, [isLoaded, selectedArticle, urlSessionId]);
...
4. 不要输出类似“根据搜索结果”、“【source】”等内容。`;
        ...
        body: JSON.stringify({
          ...
          let content = aiTextFromResponse(data);
          if (!content) throw new Error("AI 未返回文章内容。");
...
${fullText}`;
      ...
      const content = aiTextFromResponse(data);
      if (!content) throw new Error("AI 未返回段落列表。");
...
请必须以严格的 JSON 格式返回结果，不要包含任何额外说明或Markdown代码块标签：
{
  ...
  "finalPrompt": "如果 isPromptReady 为 true，在此输出最终的画面提示词（必须是中文描述，直接用于AI生图）；否则必须为空"
      ...
      const content = aiTextFromResponse(data);
      if (!content) throw new Error("AI 未返回阅读引导。");
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      if (isRecord(data) && typeof data.error === "string") throw new Error(data.error);
      const image = aiImageFromResponse(data);
