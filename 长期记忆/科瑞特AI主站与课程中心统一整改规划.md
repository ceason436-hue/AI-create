...
- `[IN_PROGRESS / Phase 3]` 运营后台现支持课件上传、预览处理状态、送审、发布、下线和失败重试；新增 `/admin/curriculum` 维护课程模块、课时、课时任务和 AI 工具绑定。课程、报名和内容 CMS 的基础 CRUD 与审计已具备，咨询线索处理、完整内容版本恢复和真实运营数据录入仍需继续完成。
