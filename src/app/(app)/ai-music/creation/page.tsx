...
function textFromAiResponse(value: unknown) {
  const response = value && typeof value === "object" ? value as { result?: { preview?: { text?: unknown } } } : null;
  return typeof response?.result?.preview?.text === "string" ? response.result.preview.text : "";
}
...
export default function MusicCreationPage() {
  ...
  const handleMagicLyrics = async () => {
    ...
    try {
      ...
      if (data && typeof data === "object" && typeof (data as { error?: unknown }).error === "string") throw new Error((data as { error: string }).error);
      const generatedText = textFromAiResponse(data);
      if (generatedText) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (createData && typeof createData === "object" && typeof (createData as { error?: unknown }).error === "string") {
        throw new Error((createData as { error: string }).error);
      }

      const musicPreview = createData && typeof createData === "object" ? (createData as { requestId?: unknown; result?: { preview?: { audioUrl?: unknown } } }).result?.preview : null;
      if (typeof musicPreview?.audioUrl === "string" && musicPreview.audioUrl) {
        const audioUrl = musicPreview.audioUrl;
        setIsGenerating(false);
        setGeneratingStatus("");
        ...
        setTracks(prev => prev.map(t => {
          if (t.id === tempId) {
            return {
              ...t,
              id: typeof (createData as { requestId?: unknown }).requestId === "string" ? (createData as { requestId: string }).requestId : Date.now().toString(),
              title: songName || "未命名 AI 原创",
              duration: "00:00",
              tags: ["AI原创", "最新"],
              cover: "/tu1.jpg",
