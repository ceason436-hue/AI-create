...
const route = z.string().trim().regex(/^\/(?:[a-z0-9-]+(?:\/[a-z0-9-]+)*)?$/i);
...
export const DEFAULT_HOME_CONTENT: HomePageContent = {
  ...
  carousel: [
    { image: "/media/reference-v4/hd/home-hero-seamless.png", alt: "少年在工作台协作搭建机器人", href: "/courses" },
    { image: "/media/site-v3/home/hero-maker-classroom-v2.png", alt: "学生在创客教室协作制作智能小车", href: "/tools/ai-code", eyebrow: "AI 创作 · 让想法动起来", titleLine1: "把好奇心变成", titleLine2: "可以运行的作品" },
  ],
};
...
export function homePageContent(sections: PublishedSection[]): HomePageContent {
  return {
    ...
    carousel: parseSection(sections, "HERO_CAROUSEL", homePageContentSchema.shape.carousel, DEFAULT_HOME_CONTENT.carousel),
  };
}
...
export function isValidManagedSectionPayload(pageKey: string, sectionType: string, payload: unknown) {
  const schema = pageKey === "home"
    ? sectionType === "HERO" ? homePageContentSchema.shape.hero : sectionType === "CTA" ? homePageContentSchema.shape.cta : sectionType === "HERO_CAROUSEL" ? homePageContentSchema.shape.carousel : null
