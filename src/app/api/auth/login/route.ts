...
import { createSession, createTemporarySchoolSession } from "@/lib/auth";
...
export async function POST(request: Request) {
  try {
    ...
    if (isTemporaryDeployment()) {
      if (parsed.data.mode !== "school" || parsed.data.loginIdentifier !== TEMPORARY_SCHOOL_ACCOUNT.loginIdentifier || parsed.data.password !== TEMPORARY_SCHOOL_ACCOUNT.password) {
        return invalidCredentials();
      }
      await createTemporarySchoolSession();
      const response = Response.json({ account: { type: "SCHOOL_SHARED", loginIdentifier: TEMPORARY_SCHOOL_ACCOUNT.loginIdentifier } });
      (await cookies()).set("krt_account_type", "SCHOOL_SHARED", { httpOnly: false, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: accountTypeCookieMaxAge("SCHOOL_SHARED") });
      return response;
    }
