...
export async function getPublicAiTools(): Promise<PublicAiTool[]> {
  if (isTemporaryDeployment()) {
    return (["code", "image", "music", "reading"] as const).map((toolKey) => {
      const base = AI_TOOL_CATALOG[toolKey];
      return { toolKey, name: base.name, description: base.description, category: base.category, routePath: base.routePath, color: base.color, cover: null };
    });
  }
