...
import { isTemporaryDeployment } from "./temporary-deployment";
...
export async function loadPublicMediaSlots(slotKeys: string[]) {
  if (isTemporaryDeployment()) return { state: "unavailable" as const, slots: {} as Record<string, PublicMediaSlot> };
  try {
