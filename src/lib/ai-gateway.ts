...
async function canUseTool(accountId: string, tool: AiTool) {
  ...
  if (account.type === AccountType.ADMIN) {
    // 管理账号需要能验收课程检索与阅读工具；仍要求账户拥有有效管理角色。
...
export async function beginAiRequest(request: Request, tool: AiTool, options?: { trackRequest?: boolean }): Promise<GatewayResult> {
  ...
  if (isTemporaryDeployment()) {
    if (!sessionAccount) return { ok: false, response: unauthorized() };
    return { ok: true, access: { accountId, requestId: randomUUID(), redisKey: "", leaseToken: "", reservedCredits: 0, temporary: true, courseContext: null } };
  }
