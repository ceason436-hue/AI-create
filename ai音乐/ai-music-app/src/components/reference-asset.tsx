...
export function ReferenceAsset({src,sourceWidth,box,alt,className=""}:{src:string;sourceWidth:number;box:string;alt:string;className?:string}) {
  ...
  const optimizedSrc = src.replace(/\.png(?=$|\?)/i, ".webp");
  return <svg className={className} viewBox={box} preserveAspectRatio="xMidYMid meet" overflow="hidden" role="img" aria-label={alt}><svg x={x} y={y} width={width} height={height} style={{width,height}} viewBox={box} overflow="hidden"><image href={optimizedSrc} width={sourceWidth}/></svg></svg>;
}
