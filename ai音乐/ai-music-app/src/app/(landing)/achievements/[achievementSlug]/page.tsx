...
export default async function AchievementDetailPage({ params }: { params: Promise<{ achievementSlug: string }> }) {
  ...
  if (!item) notFound();
  return <PublicPage eyebrow={item.type || "WORK"} title={item.title} intro={item.summary}><section className="detail-content"><PlaceholderImage src={item.coverAssetId || "/tu2.png"} alt={item.title} /><div className="rich-copy"><p>{item.content}</p><p className="placeholder-warning">当前内容包含品牌占位，不代表真实学员、学校、竞赛或获奖证据。</p></div></section></PublicPage>;
}
