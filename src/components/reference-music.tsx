...
import { stripMinimaxThinking } from "@/lib/minimax-response";
...
export function ReferenceMusic() {
  ...
  const audio = useRef<HTMLAudioElement>(null),
    ...
    lyricsDialog = useRef<HTMLDialogElement>(null);
  useEffect(() => {
  ...
  async function polish() {
    ...
    try {
      ...
      const polished = stripMinimaxThinking(text).slice(0, 1000);
      if (!polished) throw Error("歌词服务没有返回可用的歌词。");
      setLyrics((previous) => (previous === lyrics ? polished : previous));
    } catch (e) {
  ...
  async function generate() {
    ...
    try {
      const prompt = [
        style.trim(),
        genre ? `曲风：${genre}` : "",
        selected.length ? `乐器：${selected.join("、")}` : "",
      ]
        .filter(Boolean)
        .join("\n");
      ...
      if (personal) {
        ...
        const imported = await fetch("/api/works/import-audio", {
            ...
            body: JSON.stringify({
              ...
              title: track.title,
              requestId,
              parentWorkId: current?.audioUrl && personal ? current.id : undefined,
            }),
          }),
  ...
  return (
    <main className={s.page} aria-label="AI音乐创作工作台">
        ...
        <section className={s.form} aria-label="音乐创作设置">
          ...
          <div className={s.field}>
            <div className={s.labelRow}>
              ...
              <div>
                <button
                  onClick={() => lyricsDialog.current?.showModal()}
                  aria-label="在大窗口中编辑歌词"
                >
                  <Maximize2 />
                  放大编辑
                </button>
        ...
        <section className={s.results} aria-label="音乐作品">
          ...
          {tab === "当前作品" ? (
            <>
              ...
              {current && <div className={s.divider}>当前作品</div>}
              {current && (
                        ...
                        {Array.from({ length: 150 }, (_, i) => {
                          ...
                          return (
                            <line
                              ...
                              y2={38 + h / 2}
                              stroke={
                                i < 150 * (duration ? time / duration : 0)
                                  ? "#1437b0"
                                  : "#cbd5f7"
                              }
                              strokeWidth="1.8"
      ...
      <dialog ref={lyricsDialog} className={`${s.dialog} ${s.lyricsDialog}`}>
        <div className={s.dialogHeader}>
          <div>
            <h2>歌词编辑器</h2>
            <p>在大窗口中检查段落和结构，修改会自动同步。</p>
          </div>
          <button onClick={() => lyricsDialog.current?.close()}>完成</button>
        </div>
        <textarea
          value={lyrics}
