...
export default function AIArtGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const result = data && typeof data === "object" ? data as { error?: unknown; result?: { preview?: { image?: unknown } } } : {};

      if (!response.ok) {
        throw new Error(typeof result.error === "string" ? result.error : '生成失败');
      }
      if (typeof result.result?.preview?.image !== "string") throw new Error("生成服务未返回图片。");
      const generatedImage = result.result.preview.image;
