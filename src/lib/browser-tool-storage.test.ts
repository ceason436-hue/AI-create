...
import { CLASSROOM_STORAGE_TTL_MS, createMemoryStorage } from "./tool-storage";
...
describe("browser tool storage", () => {
  ...
  it("uses server-verified school identity and enforces the 12-hour session boundary", () => {
    const local = createMemoryStorage();
    const session = createMemoryStorage();
    let now = 1_000;
