...
- 2026-09-02 实施记录：运营后台新增 `/admin/media` 图片媒体库，图片经管理员鉴权上传到现有存储驱动，需记录来源类型、替代文本与授权说明；对外只由已启用媒体的 `/api/media/:id` 受控地址读取。活动、成果、师资、校区和合作学校创建时可绑定媒体资源，公开内容页采用动态读取，发布后无需重新构建。当前仅支持图片；视频、响应式派生图、多图排序、裁切焦点、媒体版本恢复与最终授权核验待后续阶段完成。
- 2026-09-02 实施记录：媒体库已扩展为图片与视频媒体库，支持 MP4、WebM 的服务端签名校验、受控存储及公开代理播放；公开活动和成果页按媒体 MIME 类型展示视频。视频上传限制可由 `MEDIA_VIDEO_UPLOAD_MAX_BYTES` 收紧，默认 200MB；转码、HLS、字幕、封面抽帧与多尺寸派生图仍待生产媒体处理服务。
- 2026-09-02 实施记录：新增媒体位管理 `/admin/media-slots`，运营人员可绑定桌面和移动端媒体、设置推荐比例与裁切焦点，并控制品牌占位、隐藏和已发布状态。首页首屏、作品卡、校园合作区及校园合作页通过媒体位读取已发布素材，媒体资产下线或未绑定时保持明确品牌占位，前端无需修改。
- 2026-09-02 实施记录：后台报名管理已增加 CSV/制表符批量导入预览；每行会先校验日期、重复组合、个人账户类型/启用状态和课程存在性，校验失败时整批不写入，校验通过后以事务 upsert 并写入汇总审计。预览与写入均需服务端管理员鉴权，真实运营数据导入仍待数据库环境验收。
- 2026-09-02 实施记录：课程发布改为服务端校验适合人群、封面媒体和至少一个已发布模块；公开课程接口仅返回已发布模块/课时。运营后台创建课程时可维护既有的详细介绍、年级、难度、授课方式、周期、招生状态与封面字段，真实运营发布流程仍待数据库环境验收。
- 2026-09-02 实施记录：运营人员现在可从课程列表进入单门课程的基础信息编辑页；Slug 为只读，其他已建模课程元数据受服务端管理员鉴权、校验和审计保护。模块、课时和工具绑定仍在独立课程结构页面维护。
- 2026-09-02 实施记录：课件上传表单改用管理员接口返回的课程和对应课时选择器，避免手工输入 ID；服务端仍在实际上传时复核课程与课时归属，不能由客户端绕过。
- 2026-09-02 实施记录：课程结构后台已补齐模块、课时和课程工具绑定的编辑、状态控制与排序输入；持久化仍通过服务端所属课程校验、字段白名单与审计日志执行。
- 2026-09-02 实施记录：课程分类后台已补齐名称、介绍、排序和启用/归档编辑；分类 Slug 固定以保护公开课程筛选地址，已有课程分类不提供物理删除操作。
- 2026-09-02 实施记录：后台单条报名不再要求运营人员手工输入账户和课程 ID；管理员页面只加载启用个人账户的最小识别字段与可选课程。提交前会把本地日期时间转换为 ISO 格式，服务端复用批量导入的账户类型、启用状态、课程存在性与有效期校验；账户选择接口和报名写入均需管理员会话。
- 2026-09-02 实施记录：已删除公开课程的具体课程、模块与课时硬编码回退；无数据库或无已发布课程时只显示整理中状态和咨询入口。课程分类、课程介绍、章节与课件必须由运营后台维护和发布，不再用前端示例数据替代。
- 2026-09-02 实施记录：首页学习过程区块的默认课程行动入口已统一跳转 `/courses`，不再链接已移除的示例课程；已发布的页面区块文案继续由运营后台管理。
- 2026-09-02 实施记录：已静态核验 44 个运营后台 API 均有管理员或超级管理员会话校验；学校管理读取、修改、删除和密码重置端点在真实未登录请求下均返回 `401`，未把后台页面本身可访问误认为数据操作已授权。
- 2026-09-02 实施记录：课件预览会话与内容端点在真实未登录请求下均返回 `401`；登录后仍必须通过有效报名、已发布课件及五分钟预览会话校验，预览内容以私有、不可下载入口方式响应。真实 OSS 与课件版式验收仍待部署环境恢复。
- 2026-09-02 实施记录：首页课程方向已改为直接读取运营后台启用的分类；分类可先于具体课程维护，尚无分类时显示明确空状态，不再使用前端示例分类填充页面。
- 2026-09-02 实施记录：站点地图现自动收录后台已发布课程、活动和成果详情；数据库不可用时只保留固定公开页，绝不收录草稿、占位内容或已移除的示例课程。
- 2026-09-02 实施记录：活动、成果、师资和校区的品牌占位回退现统一显示 `[PENDING-CONTENT]` 来源标签；手册资料继续明确为真实资料但待授权/展示范围确认，避免公开页误把占位或手册内容宣传为已核验事实。
...
| 2026-09-02 | 单条报名与批量导入使用同一报名校验策略 | 避免两条运营路径在账户身份、账户状态、课程存在性或有效期校验上出现差异；账户选择接口仅向管理员返回最小字段。 |
| 2026-09-02 | 公开课程不再使用具体示例回退 | 课程分类、课程、章节和课件由运营后台单一维护；数据库不可用或尚无发布内容时明确显示空状态，避免把未确认示例误作真实课程。 |
...
- 主站内容运营新增 `ContentItemRevision`：活动、成果、师资、校区与合作学校在新建、编辑、发布状态切换和恢复时都会保存不可变快照；恢复受字段白名单约束、不会删除原版本，并在同一事务内写入审计。新迁移 `20260902093000_content_item_revisions` 需要在下一次受控数据库部署中执行。
- 媒体库新增 `MediaAssetRevision`：上传媒体、修改可发布元数据或恢复历史都会生成版本与审计；二进制对象不会因元数据恢复而被覆盖。新迁移 `20260902094000_media_asset_revisions` 需要在下一次受控数据库部署中执行。
- 内容运营新增 `ContentMedia`：活动、成果、师资、校区和合作学校可将多个已启用媒体按封面与顺序编排，附带说明和裁切焦点；解除关联不会删除媒体对象，所有变更写入审计。新迁移 `20260902095000_content_media` 需要在下一次受控数据库部署中执行。
- 公开活动与成长详情页已读取 `ContentMedia` 图集：仅发布内容关联的启用媒体可见，封面与排序由后台控制；已下线或解除关联的媒体不会继续公开展示。
...
- 单条报名选择器已完成本地无管理员会话下的页面与 401 权限回归；真实管理员账户、课程数据和数据库写入成功路径仍须在受控环境验收。
