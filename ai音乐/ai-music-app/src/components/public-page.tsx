...
export function ContentGallery({ items, alt }: { items?: Array<{ src: string; mimeType: string | null; caption: string | null; focalPoint: string | null; captionsSrc?: string | null; captionLanguage?: string | null }>; alt: string }) {
  ...
  return <div className="content-gallery">{items.map((item, index) => <figure key={`${item.src}-${index}`} style={{ objectPosition: item.focalPoint || "center" }}>{item.mimeType?.startsWith("video/") ? <video src={item.src} controls playsInline preload="metadata" aria-label={`${alt}素材 ${index + 1}`}>{item.captionsSrc && <track kind="captions" src={item.captionsSrc} srcLang={item.captionLanguage || "zh-CN"} label="字幕" default />}</video> : <Image src={item.src} alt={item.caption || `${alt}素材 ${index + 1}`} width={1000} height={650} />} {item.caption && <figcaption>{item.caption}</figcaption>}</figure>)}</div>;
}
