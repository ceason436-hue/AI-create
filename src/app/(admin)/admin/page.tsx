...
export default function AdminDashboardPage() {
  ...
  const [delegatedPermissions, setDelegatedPermissions] = useState<string[] | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const session = await sessionResponse.json().catch(() => null) as { account?: { roleKeys?: string[] } } | null;
      const roleKeys = session?.account?.roleKeys ?? [];
      if (!sessionResponse.ok || !session?.account) {
        router.replace("/login?mode=admin&next=/admin");
        return;
      }
      if (!roleKeys.includes("SUPER_ADMIN")) {
        setDelegatedPermissions(roleKeys);
        setData({ schools: [], cohorts: [], plans: [], invitations: [], costs: null });
        return;
      }
      setDelegatedPermissions(null);
      const urls = [
        "/api/admin/schools",
  ...
  if (delegatedPermissions) {
    const destinations = [
      { key: "ADMIN_USERS", label: "账号与报名", href: "/admin/enrollments", copy: "管理学员报名和账号数据" },
      { key: "ADMIN_COURSES", label: "课程与课件", href: "/admin/courses", copy: "维护课程、课时、公告和课件" },
      { key: "ADMIN_CONTENT", label: "内容与媒体", href: "/admin/content", copy: "维护公开站点内容与媒体资源" },
      { key: "ADMIN_AI", label: "AI 工具设置", href: "/admin/ai-tools", copy: "启停工具并维护生成规则" },
      { key: "ADMIN_INQUIRIES", label: "咨询线索", href: "/admin/inquiries", copy: "查看并跟进课程与合作咨询" },
    ].filter((item) => delegatedPermissions.includes(item.key));
    return <main className="min-h-screen bg-slate-100 p-6 text-slate-900 lg:p-10"><div className="mx-auto max-w-5xl"><div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-xl bg-blue-950 text-white"><ShieldCheck /></div><div><h1 className="text-2xl font-black">管理工作台</h1><p className="mt-1 text-sm text-slate-600">只显示已授权的管理区域。</p></div></div><div className="mt-7 grid gap-4 md:grid-cols-2">{destinations.map((item) => <Link key={item.key} href={item.href} className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:ring-blue-300"><h2 className="font-black text-blue-950">{item.label}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{item.copy}</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-blue-700">进入管理 <ChevronRight size={16} /></span></Link>)}</div></div></main>;
  }
  const selectedNav =
  ...
  return (
    <section className="flex min-h-screen w-full overflow-visible bg-[#f3f6fb] text-[#24324d] lg:flex-row">
      <aside className="w-full shrink-0 border-b border-[#273982] bg-[#172569] text-white lg:sticky lg:top-0 lg:h-screen lg:w-64 lg:self-start lg:overflow-y-auto lg:border-b-0 lg:border-r">
        <div className="flex items-center gap-3 border-b border-[#33458f] px-5 py-5">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#f05a24] text-white">
            <Settings2 size={21} />
          </div>
          <div>
        ...
        <nav className="flex gap-1 overflow-x-auto p-3 lg:block">
          {navigation.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`flex min-w-max items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition lg:mb-1 lg:w-full ${active === id ? "bg-[#f05a24] text-white" : "text-slate-200 hover:bg-[#273982] hover:text-white"}`}
            >
              <Icon size={18} />
              <span>{label}</span>
          ...
          <Link
            href="/admin/admins"
            className="mt-1 flex min-w-max items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold text-slate-200 transition hover:bg-[#273982] hover:text-white lg:w-full"
          >
            <ShieldCheck size={18} />
            <span>管理员与权限</span>
            <ChevronRight size={15} className="ml-auto hidden lg:block" />
        ...
        <div className="hidden border-t border-[#33458f] p-4 text-xs leading-5 text-slate-300 lg:block">
          权限受服务端会话保护
          <br />
          敏感操作会写入审计日志
        </div>
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-10 border-b border-[#d9e0ee] bg-[#f8fafd]/95 px-5 py-4 backdrop-blur lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#62708b]">
                {selectedNav.label}
              </p>
              <h1 className="mt-1 text-2xl font-black tracking-tight text-[#1d2f82]">
                {subTab}
              </h1>
            </div>
        ...
        <div className="flex flex-wrap gap-2 border-b border-slate-200 bg-slate-50 px-5 py-3 lg:px-8">
          <Link href="/admin/resources" className="mr-2 inline-flex items-center rounded-lg bg-[#1d2f82] px-3 py-2 text-sm font-bold text-white transition hover:bg-[#172569]">内容与学习</Link>
          <Link href="/admin/resources" className="inline-flex items-center px-1 text-sm font-bold text-[#1d2f82] underline decoration-[#f05a24] decoration-2 underline-offset-4">资源工作面板</Link>
          ...
          <Link href="/admin/ai-tools" className="rounded-lg bg-white px-3 py-2 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-[#caf204]">AI 工具目录</Link>
          ...
          <Link href="/admin/reading-lessons" className="rounded-lg bg-white px-3 py-2 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-[#caf204]">AI 阅读课文库</Link>
